...
- **记录页「每日花费」日历**(2026-09-07):月历网格显示每天支出,点选日期联动当日明细。

### 识屏智能体(实验,「咨询」Tab,2026-09-07 整合自 sult_liban,MIT)
- 第二条感知通道:MediaProjection 录屏 + MLKit 中文 OCR 读取任意购物页 → 技能决策
  (预算占用/储蓄目标延迟/冲动信号 + DeepSeek 模型估价) → 悬浮结果卡「还是买/等等/放弃」;
- 与无障碍记账互不依赖;数据仍只存本机(agent.db);智谱搜索代码保留未启用。
